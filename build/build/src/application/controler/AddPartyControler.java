...
public class AddPartyControler implements Initializable {
    ...
    void save(ActionEvent event) {
    	try {
			...
			PurchaseParty party = new PurchaseParty(txtPartyName.getText(), 
					txtAddress.getText(), txtGst.getText(), txtPanno.getText());
			party.setId(id);
    ...
    private int validateData()
    {
    	try {
			...
			{
				new Alert(AlertType.ERROR,"Enter Party Name !!!").showAndWait();
				txtPartyName.requestFocus();
				return 0;
			}
			if(txtAddress.getText().equals("")|| txtAddress.getText()==null)
			{
				new Alert(AlertType.ERROR,"Enter Part Address!!!").showAndWait();
